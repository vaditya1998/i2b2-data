...
BEGIN
    ...
    ELSIF p_agg_type = 'NumEncounters' THEN
        v_aggregation_sql := 'COUNT(DISTINCT ENCOUNTER_NUM)::TEXT val from t group by patient_num';
    ELSIF p_agg_type = 'NumConcepts' THEN
        v_aggregation_sql := 'COUNT(DISTINCT CONCEPT_CD)::TEXT val from t group by patient_num';
    ELSIF p_agg_type = 'NumProviders' THEN
        v_aggregation_sql := 'COUNT(DISTINCT PROVIDER_ID)::TEXT val from t where PROVIDER_ID <> ''@'' group by patient_num';
    ELSIF p_agg_type = 'NumDates' THEN
        v_aggregation_sql := 'COUNT(DISTINCT DATE(START_DATE))::TEXT val from t group by patient_num';
    ELSIF p_agg_type = 'NumFacts' THEN
        v_aggregation_sql := 'COUNT(*)::TEXT val from t group by patient_num';
    ELSIF p_agg_type = 'NumValues' THEN
        v_aggregation_sql := 'COUNT(DISTINCT NVAL_NUM)::TEXT val from t group by patient_num';
    ELSIF p_agg_type = 'MinDate' THEN
        v_aggregation_sql := 'MIN(START_DATE)::TEXT val from t group by patient_num';
    ELSIF p_agg_type = 'MaxDate' THEN
        v_aggregation_sql := 'MAX(START_DATE)::TEXT val from t group by patient_num';
    ELSIF p_agg_type = 'MinValue' THEN
        v_aggregation_sql := 'MIN(NVAL_NUM)::TEXT val from t group by patient_num';
    ELSIF p_agg_type = 'MaxValue' THEN
        v_aggregation_sql := 'MAX(NVAL_NUM)::TEXT val from t group by patient_num';
    ELSIF p_agg_type = 'AvgValue' THEN
        v_aggregation_sql := 'AVG(NVAL_NUM)::TEXT val from t group by patient_num';
    ELSIF p_agg_type = 'MedianValue' THEN
        v_aggregation_sql := 'PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY NVAL_NUM) OVER (PARTITION BY PATIENT_NUM)::TEXT val from t';
    ELSIF p_agg_type = 'FirstValue' THEN
        v_aggregation_sql := 'NVAL_NUM::TEXT val FROM (select PATIENT_NUM, ROW_NUMBER() OVER (PARTITION BY PATIENT_NUM ORDER BY START_DATE ASC) rn, NVAL_NUM from t) x where rn = 1';
    ELSIF p_agg_type = 'LastValue' THEN
        v_aggregation_sql := 'NVAL_NUM::TEXT val FROM (select PATIENT_NUM, ROW_NUMBER() OVER (PARTITION BY PATIENT_NUM ORDER BY START_DATE DESC) rn, NVAL_NUM from t) x where rn = 1';
    ...
    ELSIF p_agg_type = 'MedianDate' THEN
        v_aggregation_sql := ' (min(START_DATE) + ((max(START_DATE) - min(START_DATE)) / 2))::TEXT val FROM ( ' ||
    ...
    ELSIF p_agg_type = 'MultiACTDrugIng' THEN
        v_aggregation_sql := 'REPLACE(CONCEPT_NAME, '','', '''') col, COUNT(DISTINCT START_DATE)::TEXT val ' ||
