...
BEGIN
    ...
    IF p_CONSTRAIN_BY_DATE_FROM IS NOT NULL THEN
        v_SQLConstraint := v_SQLConstraint || ' AND START_DATE >= TO_DATE(''' || p_CONSTRAIN_BY_DATE_FROM || ''', ''YYYY-MM-DD'')';
    END IF;

    IF p_CONSTRAIN_BY_DATE_TO IS NOT NULL THEN
        v_SQLConstraint := v_SQLConstraint || ' AND START_DATE <= TO_DATE(''' || p_CONSTRAIN_BY_DATE_TO || ''', ''YYYY-MM-DD'')';
    END IF;
    ...
    IF p_C_TABLENAME = 'concept_dimension' THEN
        ...
        ELSIF p_CONSTRAIN_BY_VALUE_TYPE = 'ENUM' THEN
            IF p_CONSTRAIN_BY_VALUE_OPERATOR = 'IN' THEN
                v_SQLConstraint := v_SQLConstraint || ' AND TVAL_CHAR IN ' || p_CONSTRAIN_BY_VALUE_CONSTRAINT;
            ELSE
